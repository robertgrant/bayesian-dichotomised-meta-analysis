...
for (i in 1:t) {
  ...
  mc1[i]<-mean(temp.c[,2])
  sc1[i]<-sd(temp.c[,2])
  rc1[i]<-sum(temp.c[,2]<=threshold)
  mt1[i]<-mean(temp.t[,2])
  st1[i]<-sd(temp.t[,2])
  rt1[i]<-sum(temp.t[,2]<=threshold)
}
rm(temp.c)
rm(temp.t)
...
real threshold; // absolute threshold
...
sdtau = 1/sqrt(tau);
for (i in 1:td)
riskt1[i] = Phi_approx((threshold-(mu0+u[tk+i]+dt))/st1[tk+i]);
for (i in 1:td)
riskc1[i] = Phi_approx((threshold-(mu0+u[tk+i]+dc))/sc1[tk+i]);
diff<-(dt-dc);
}
model {
for (i in 1:t)
...
# abs2fits is a list of the R objects pointing to the compiled models
# abs1fits likewise, for the baseline-data-only models
abs1fits<-abs2fits<-list(NULL)
...
ma2data<-list(t=t,armsize=armsize,tk=tk,td=td,
              ...
              mc1=mc1[1:tk],sc1=sc1,rc1=rc1[(tk+1):t],
              mt1=mt1[1:tk],st1=st1,rt1=rt1[(tk+1):t])
abs2fits[[1]]<-stan(model_code=ma2model,data=ma2data,iter=staniter,
                   init=ma2inits,chains=2)
abs2fits[[2]]<-fit2 # This already exists
...
ma1inits<-list(
  ...
  init2=list(mu0=158,dc=-3,dt=-14,tau=0.1,
             u=seq(from=-2/sqrt(0.1),
                   to=2/sqrt(0.1),
                   length.out=tk)))
abs1fits[[1]]<-stan(model_code=ma1model,data=ma1data,iter=staniter,
                   init=ma1inits,chains=2)
abs1fits[[2]]<-fit1 # This already exists
...
for (td in 4:8) {
  ...
  ma2data<-list(t=t,armsize=armsize,tk=tk,td=td,
                ...
                mc1=mc1[1:tk],sc1=sc1,rc1=rc1[(tk+1):t],
                mt1=mt1[1:tk],st1=st1,rt1=rt1[(tk+1):t])
  abs2fits[[td-1]]<-stan(model_code=ma2model,data=ma2data,
  ...
  ma1inits<-list(
    ...
    init2=list(mu0=158,dc=-3,dt=-14,tau=0.1,
               u=seq(from=-2/sqrt(0.1),
                     to=2/sqrt(0.1),
                     length.out=tk)))
  abs1fits[[td-1]]<-stan(model_code=ma1model,data=ma1data,
...
simabs<-function(simID=1,nsims=1,armsize=100,threshold=140,effectsize=(-10),
                sim1fits=abs1fits,sim2fits=abs2fits,
      ...
      ma1inits<-list(init1=list(mu0=154,dc=-5,dt=-11,tau=0.08,
                     ...
                     init2=list(mu0=158,dc=-3,dt=-14,tau=0.1,
                                u=seq(from=-2/sqrt(0.1),
                                      to=2/sqrt(0.1),
                                      length.out=tk)))
      new2fit<-invisible(stan(fit=abs2fits[[td-1]],data=ma2data,init=ma2inits,
                              iter=staniter,chains=stanchains,refresh=0,seed=seed+2000))
      new1fit<-invisible(stan(fit=abs1fits[[td-1]],data=ma1data,
                              iter=staniter,chains=stanchains,refresh=0,seed=seed+1000))
...
# test run
# system.time(simabs(nsims=3))
# system.time(lapply(1:3,simabs,nsims=2))
# abs <- mclapply(X=1:3, FUN=simabs, nsims=33, mc.cores=3)

# parallel run
start_time <- Sys.time()
abs_sims <- mclapply(X=1:6, FUN=simabs, nsims=2000, mc.cores=6)
end_time <- Sys.time()
save.image('absolute-armsize100.RData')
